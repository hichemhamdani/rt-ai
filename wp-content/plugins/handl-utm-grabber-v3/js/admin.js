jQuery(function($) {
    ...
    jQuery(document).ready(function($) {
        ...
        const cookieDurationInput = $('#cookie_duration');
        const cookieDurationDesc = $('#cookie-duration-description');
        const sessionCookiesCheckbox = $('#enable_session_cookies');
        
        function toggleCookieDuration() {
            const isSessionCookies = sessionCookiesCheckbox.is(':checked');
            cookieDurationInput.prop('disabled', isSessionCookies);
            
            if (isSessionCookies) {
                cookieDurationDesc.text('Cookie duration is disabled because Session Cookies is enabled. Cookies will expire when the browser session ends.');
            } else {
                cookieDurationDesc.text('Specify how many days the cookies should persist.');
            }
        }
        
        sessionCookiesCheckbox.on('change', toggleCookieDuration);
        toggleCookieDuration(); // Run on page load
    });
