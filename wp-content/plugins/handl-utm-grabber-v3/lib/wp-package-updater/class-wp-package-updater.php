...
if ( ! class_exists( 'WP_Package_Updater' ) ) {
	...
	class WP_Package_Updater {
		...
		public function __construct(
			...
			$this->set_type($package_file_path);
			...
			$metadata_url  = $this->update_server_url . '?action=get_metadata&package_id=';
			...
			if ( $this->use_license ) {
				$this->license_server_url = $update_server_url;
		...
		public function add_admin_scripts( $hook ) {
			...
			if ( $condition ) {
//				$js_ext = ( $debug ) ? '.js' : '.min.js'; //not necessery, this is in Admin already
                $js_ext = ( $debug ) ? '.js' : '.js';
		...
		public function filter_update_checks( $query_args ) {
            ...
            $query_args['domain'] = defined('PANTHEON_ENVIRONMENT') ? $_SERVER['HTTP_HOST'] : $_SERVER['SERVER_NAME']; //PANTHEON license fix
		...
		protected function do_query_license( $query_type ) {
			...
			$api_params = array(
				...
				'allowed_domains' => defined('PANTHEON_ENVIRONMENT') ? $_SERVER['HTTP_HOST'] : $_SERVER['SERVER_NAME'], //PANTHEON license fixs
		...
		protected static function is_plugin_file( $absolute_path, $package_file_path ) {
			...
			if ( function_exists( 'get_file_data' ) ) {
				$headers = get_file_data( $package_file_path, array( 'Name' => 'Plugin Name' ), 'plugin' );
		...
		protected function set_type($package_file_path) {
			$theme_directory = self::get_theme_directory_name( $this->package_path );
			$this->type = 'Plugin';
//			if ( self::is_plugin_file( $this->package_path, $package_file_path ) ) {
//				$this->type = 'Plugin';
