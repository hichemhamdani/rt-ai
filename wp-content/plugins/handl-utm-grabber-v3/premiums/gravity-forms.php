<?php
require_once plugin_dir_path( __FILE__ ) . '../includes/utils/class-onboarding-tracker.php';
                    ...
                    <td>
                        ...
                        <?php foreach($forms as $form): ?>
                            <fieldset>
                                ...
                                <label for="forms[]">
                                    <input name="forms[]" type="checkbox" class="forms" value="<?php print $form['id']; ?>" <?php print $handl_fields_disabled; ?>>
                                    <?php print $form['title'] . " (" . $form['id'] . ")"; ?>
                                </label>
                            </fieldset>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <p>
                            <em>
                                No Gravity Forms found. Please create a form or import your Gravity Forms 
                                <a href="<?php echo admin_url('admin.php?page=gf_edit_forms'); ?>">here</a> 
                                to continue.
                            </em>
                        </p>
                    <?php endif; ?>
...
//            dd($myforms);
            ...
            if ($result) {
                ...
                if ($_POST['gf__action'] == 'add'){
                    $tracker->complete_step('gravity_forms');
                }
                elseif ($_POST['gf__action'] == 'remove'){
                    $tracker->reset_onboarding('gravity_forms');
                }
            }else{
	    ...
	    foreach ($form['fields'] as $field){
		    foreach ($utmfields as $utmfield){
			    if ($field['inputName'] === $utmfield){
				    ...
				    // Use attribute selector [id="..."] to handle multiple elements with same ID
				    // This will select ALL elements with that ID, not just the first one
				    $script .= "Cookies.get(\"$utmfield\") ? jQuery('[id=\"$input_id\"]').val(decodeURIComponent(sanitizeInput(Cookies.get(\"$utmfield\")) ?? '')) : '';\n";
			    }
		    }
	    }
