...
//	WC_Subscriptions_renewal::trigger_renewal_payment(3859);
//    dd($wooPostbacks);
                        ...
                        <label for='custom_params'>
                            <select name="woo_postback[0][template]" class="preload_template" id="preload_template_0" data-level="0">
                                ...
                                foreach (array(
                                        ...
                                        'fb' => 'Facebook Ads (Offline Conversion) - This is deprecated, use Facebook CAPI tab to enable this feature',
                                         ) as $value => $text):
                                ?>
...
//        'renewal_order_payment_complete'
    );
}
				...
				if ( ! is_array( $value ) && preg_match( '/handl\|(.+)/', $value, $output ) ) {
					...
					if ( sizeof( $output ) == 2 ) {
						...
						if ( $keyName == 'data' ) {
                            ...
                            if (sizeof($fieldNames) == 1 && $fieldNames[0] == "subtotal"){
	                            $args[ $key ] = $order->get_subtotal();
                            }else{
...
function HandLWooCommercePostback($order_id, $hook){
    ...
    if (isset($wooPostbacks[0]) && $wooPostbacks[0][$hook] != ''){
        ...
        // DEPRECATED: This method of sending offline conversions will be removed in a future version.
        // use Facebook CAPI tab to enable this feature
        if ($template == 'fb'){
