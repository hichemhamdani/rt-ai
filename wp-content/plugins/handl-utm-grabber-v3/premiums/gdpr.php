		...
		<td>
			<fieldset>
				...
				<label for='show_gdpr_notice'>
                    ...
                    <span class="description">This will display HandL's consent bar in your website. Our plugin won't collect any data until consent is given. If you are using other consent plugins, you do not need to enable this. </span>
				</label>
			</fieldset>
		</td>
	</tr>
...
function register_third_party_consent_field() {
    register_setting('handl-utm-grabber-settings-group', 'require_third_party_consent');
}
add_action('admin_init', 'register_third_party_consent_field');

function add_third_party_consent_field() {
    global $handl_fields_disabled;
    ?>
    <tr>
        <th scope='row'>Require Third-Party Consent</th>
        <td>
            <fieldset>
                <legend class='screen-reader-text'>
                    <span>Require Third-Party Consent</span>
                </legend>
                <label for='require_third_party_consent'>
                    <input name='require_third_party_consent' id='require_third_party_consent' type='checkbox' value='1' <?php checked('1', get_option('require_third_party_consent')); ?> <?php echo $handl_fields_disabled; ?> />
                    Require consent from third-party services (e.g., Google Tag Manager) before tracking
                </label>
            </fieldset>
        </td>
    </tr>
...
add_filter("insert_rows_to_handl_options", "add_third_party_consent_field", 11);