...
//This encrypted line is nothing to worry about. This helps us to manage license. Tempering this line is strictly prohibited.
$file = __FILE__;
...
if ( ! function_exists( 'handl_sanitize_tracking_host' ) ) {
	function handl_sanitize_tracking_host( $host ) {
		return preg_replace( '/[^a-z0-9\.\-]/i', '', strtolower( handl_sanitize_tracking_value( $host ) ) );
	}
}

function handl_utm_grabber_v3_activated() {
	deactivate_plugins( '/handl-utm-grabber/handl-utm-grabber.php' );
    update_option('handl_utm_grabber_v3_activation_redirect', true);
}
...
if ( ! function_exists( 'CaptureUTMs' ) ) {
	function CaptureUTMs() {
		// Check Fetch Metadata headers (if present, treat non-navigation requests as skip)
		$sec_fetch_mode = isset($_SERVER['HTTP_SEC_FETCH_MODE']) ? $_SERVER['HTTP_SEC_FETCH_MODE'] : null;
		$sec_fetch_dest = isset($_SERVER['HTTP_SEC_FETCH_DEST']) ? $_SERVER['HTTP_SEC_FETCH_DEST'] : null;
		$is_fetch_metadata_skip = false;
		if ($sec_fetch_mode !== null && $sec_fetch_mode !== 'navigate') {
			$is_fetch_metadata_skip = true;
		}
		if ($sec_fetch_dest !== null && $sec_fetch_dest !== 'document') {
			$is_fetch_metadata_skip = true;
		}
		
		// Check Accept header for application/json
		$accept_header = isset($_SERVER['HTTP_ACCEPT']) ? $_SERVER['HTTP_ACCEPT'] : '';
		$is_json_accept = strpos($accept_header, 'application/json') !== false;
		
		// Check for ajax query parameter
		$has_ajax_param = isset($_GET['ajax']) && $_GET['ajax'] == '1';
		
		// Check if /api/ is in the request path
		$has_api_path = isset($_SERVER['REQUEST_URI']) && strpos($_SERVER['REQUEST_URI'], '/api/') !== false;
		...
		if ( is_admin() ||
             ...
             ($_SERVER['REQUEST_METHOD'] === 'POST' && strpos($_SERVER['REQUEST_URI'], '/wp-json/') !== false) ||
		     $is_fetch_metadata_skip ||
		     $is_json_accept ||
		     $has_ajax_param ||
		     $has_api_path
		) {
		...
		if (isset($_SERVER['HTTP_SEC_FETCH_DEST']) && $_SERVER['HTTP_SEC_FETCH_DEST'] == 'iframe'){
			$orig_url = isset($_SERVER['HTTP_REFERER']) ? handl_sanitize_tracking_value($_SERVER['HTTP_REFERER']) : '';
		...
		if ( isset($_SERVER['HTTP_REFERER']) && ! isset( $_COOKIE['handl_original_ref'] ) && !preg_match("/\.map$/", $_SERVER['HTTP_REFERER']) ) {
			$handl_original_ref = isset( $_SERVER['HTTP_REFERER'] ) ? handl_sanitize_tracking_value($_SERVER['HTTP_REFERER']) : '';
		...
		if ( ! isset( $_COOKIE['handl_landing_page'] ) && !preg_match("/\.map$/", $_SERVER['REQUEST_URI']) && isset($_SERVER["SERVER_NAME"]) ) {
			$server_name = handl_sanitize_tracking_host($_SERVER["SERVER_NAME"]);
			$request_uri = handl_sanitize_tracking_value($_SERVER["REQUEST_URI"]);
			$handl_landing_page = ( isset( $_SERVER["HTTPS"] ) ? 'https://' : 'http://' ) . $server_name . $request_uri;
			HandLCreateParameters('handl_landing_page', $handl_landing_page, $domain);
		}

		$handl_ip = '';
		if ( isset( $_SERVER["HTTP_X_FORWARDED_FOR"] ) && $_SERVER["HTTP_X_FORWARDED_FOR"] != "" ) {
			$handl_ip = handl_sanitize_tracking_value($_SERVER["HTTP_X_FORWARDED_FOR"]);
		} else {
			$handl_ip = isset($_SERVER["REMOTE_ADDR"]) ? handl_sanitize_tracking_value($_SERVER["REMOTE_ADDR"]) : '';
		}

		if ($handl_ip != ''){
            $handl_ip = trim(explode(',', $handl_ip)[0]);
			HandLCreateParameters('handl_ip', $handl_ip , $domain);
		}

		$handl_ref = isset( $_SERVER['HTTP_REFERER'] ) && !preg_match("/\.map$/", $_SERVER['HTTP_REFERER']) ? handl_sanitize_tracking_value($_SERVER['HTTP_REFERER']) : '';
		...
		if ( !preg_match("/\.map$/", $_SERVER['REQUEST_URI']) && isset($_SERVER["SERVER_NAME"]) ){
			$server_name = handl_sanitize_tracking_host($_SERVER["SERVER_NAME"]);
			$request_uri = handl_sanitize_tracking_value($_SERVER["REQUEST_URI"]);
			$handl_url = ( isset( $_SERVER["HTTPS"] ) ? 'https://' : 'http://' ) . $server_name . $request_uri;
		...
		$user_agent = isset($_SERVER['HTTP_USER_AGENT']) ? handl_sanitize_tracking_value($_SERVER['HTTP_USER_AGENT']) : '';
		if ($user_agent != ''){
...
//		var_dump($domain);die;
		foreach ( $fields as $id => $field ) {
			if(isset($_GET[$field]) && $_GET[$field] != '') {
				...
				$cookie_field = handl_sanitize_tracking_value($_GET[$field]);
			}elseif (isset($queryArgs[$field]) && $queryArgs[$field] != '') {
				// Sanitize input (sanitize early)
				$cookie_field = handl_sanitize_tracking_value($queryArgs[$field]);
			} elseif (isset($_COOKIE[$field]) && $_COOKIE[$field] != '') {
				$cookie_field = handl_sanitize_tracking_value($_COOKIE[$field]);
			} else {
				$cookie_field = '';
			}

			if ( ($field == 'organic_source' || $field == 'organic_source_str') && $cookie_field == '' && isset($_SERVER["HTTP_REFERER"]) && $_SERVER["HTTP_REFERER"] != "") {
				$cookie_field = handl_sanitize_tracking_value($_SERVER["HTTP_REFERER"]);
...
if ( ! function_exists( 'HandLOrganicSourceHref2Source' ) ) {
    function HandLOrganicSourceHref2Source($href)
    {
        ...
        $server_name = isset($_SERVER["SERVER_NAME"]) ? handl_sanitize_tracking_host($_SERVER["SERVER_NAME"]) : '';
        $request_uri = isset($_SERVER["REQUEST_URI"]) ? handl_sanitize_tracking_value($_SERVER["REQUEST_URI"]) : '';
        $this_domain = parse_url(( isset( $_SERVER["HTTPS"] ) ? 'https://' : 'http://' ) . $server_name . $request_uri, PHP_URL_HOST);
    ...
    function HandLCreateParameters($field, $cookie_field, $domain)
    {
	    ...
	    if (
            ...
            if (preg_match("/^first_/", $field) and isset($_COOKIE[$field])){
	            ...
	            $expiry = get_option('enable_session_cookies') ? 0 : time() + 60 * 60 * 24 * getHandLCookieDuration();
	            handl_setcookiesamesite($field, $cookie_field, $expiry, '/', $domain, true, false, "None");
	            $_COOKIE[ $field ] = $cookie_field;
            }
	    }
        ...
        add_filter('gform_field_value_' . $field, function () use ($cookie_field) {
...
{
    $disable_server_side_tracking = get_option('disable_server_side_tracking');
    $excluded_params = ['handl_original_ref', 'handl_landing_page', 'handl_landing_page_base', 'handl_ref', 'handl_url', 'handl_url_base' ,'organic_source', 'organic_source_str', 'traffic_source', 'first_traffic_source', 'first_utm_campaign', 'first_utm_source', 'first_utm_medium', 'first_utm_content', 'first_utm_term', 'utm_campaign', 'utm_medium', 'utm_source', 'fbclid', 'user_agent', 'handl_ip'];

	if (strpos($value, '#') !== false) {
		$value = explode('#', $value)[0];
	}

    if (!$disable_server_side_tracking || $disable_server_side_tracking == "" || ($disable_server_side_tracking && !in_array($name, $excluded_params))) {
...
if ( ! function_exists( 'HandlCreateShortcode' ) ) {
    function HandlCreateShortcode($field, $cookiefield)
    {
        ...
        add_shortcode($field, function () use ($cookiefield) {
            return esc_html($cookiefield);
        });
        add_shortcode($field . "_i", function ($atts, $content) use ($cookiefield) {
            return sprintf($content, esc_html($cookiefield));
        });
    }
}
...
if ( ! function_exists( 'getDomainName' ) ) {
    function getDomainName() {
        ...
        if ( get_option( 'handl_cookie_domain' ) || defined( 'DOING_CRON' ) ){
            ...
            $host = handl_sanitize_tracking_host($_SERVER["SERVER_NAME"]);
            ...
            if (sizeof($domainParts) == 1){
                ...
                if (getHandLTestCookie($testCookieName) != $testCookieValue){
                    ...
                    $domain = handl_sanitize_tracking_host($_SERVER["SERVER_NAME"]);
...
if ( ! function_exists( 'handl_utm_grabber_enqueue' ) ) {
	function handl_utm_grabber_enqueue() {
        ...
        wp_localize_script( 'handl-utm-grabber', 'handl_utm_session_cookies', array('enabled' => get_option('enable_session_cookies') ? 1 : 0));
        wp_localize_script('handl-utm-grabber', 'handl_ajax', array(
            'ajax_url' => admin_url('admin-ajax.php'),
            'fb_capi_enabled' => get_option('handl_fb_capi_enabled', '0'),
            'require_third_party_consent' => get_option('require_third_party_consent', false)
        ));
...
if ( ! function_exists( 'handl_utm_grabber_enqueue_admin' ) ) {
	function handl_utm_grabber_enqueue_admin() {
		wp_register_script( 'handl-utm-grabber-admin', plugins_url( '/js/admin.js', __FILE__ ), array( 'jquery', 'jquery-ui-core', 'jquery-ui-datepicker' ) );
	...
	function HUGGenerateUTMsForURL() {
		...
		foreach ( $fields as $id => $field ) {
			if ( isset( $_COOKIE[ $field ] ) && $_COOKIE[ $field ] != '' ) {
				$utms[ $field ] = urlencode(handl_sanitize_tracking_value($_COOKIE[ $field ]));
			}
		}
	...
	function HandLUTMGrabberWooCommerceUpdateOrderMeta( $order_id ) {
		...
		foreach ( $fields as $field ) {
			...
			if ( ! empty( $_POST[$field] ) ) {
				$value = handl_sanitize_tracking_value($_POST[$field]);
			}elseif( isset( $_COOKIE[ $field ] ) && $_COOKIE[ $field ] != '' ) {
				$value = handl_sanitize_tracking_value($_COOKIE[$field]);
			}
...
function handl_display_error(){
    ...
    if ( !$handl_active ): ?>
        <div id="setting-error-handl-utm-grabber-license-error" class="notice notice-error settings-error">
            <p><strong>You are currently using a limited version of the plugin, as we have been unable to verify your license key. To activate the plugin and utilize it to its full capacity, please navigate to the <a href="plugins.php">plugin menu</a> and enter your license key there.</strong></p>
        </div>
    <?php endif;
}
...
function handl_manage_license_modal($plugin_meta, $plugin_file, $plugin_data, $status){
	...
	if ( plugin_basename(__FILE__) === $plugin_file ) {
		$plugin_meta[] = sprintf(
			'<a id="license_field" href="javascript:void(0);" onclick="var elem = jQuery(\'div[data-package_slug=handl-utm-grabber-v3]\').parent(); var isHidden = elem.parent().parent().css(\'display\') === \'none\'; elem.css(\'display\', isHidden ? \'block\' : \'none\'); elem.parent().parent().css(\'display\', isHidden ? \'table-row\' : \'none\');">%s</a>',
			__( 'License' )
		);
	}
    return $plugin_meta;
    ...
    function handl_simple_admin_notice() {
        if ( isset($_GET['page']) && $_GET['page'] == 'handl-utm-grabber.php'){
            ...
            if ( isset($_GET['msg']) && $_GET['msg'] != '' ){
                $msg = handl_sanitize_tracking_value($_GET['msg']);
                if (isset($_GET['err']))
                    $class = "notice-error";
                ?>
                <div class="notice <?php print esc_attr($class);?> is-dismissible">
...
// require_once plugin_dir_path(__FILE__)."react-admin.php";
foreach (glob(plugin_dir_path(__FILE__)."includes/*.php") as $incFile) {
    require_once $incFile;
}

function handl_utm_grabber_v3_redirect() {
    if ( ! function_exists( 'is_plugin_active' ) ) {
        require_once ABSPATH . 'wp-admin/includes/plugin.php';
    }
    
    if (get_option('handl_utm_grabber_v3_activation_redirect', false)) {
        // Delete the flag so the redirect only happens once
        delete_option('handl_utm_grabber_v3_activation_redirect');
        ...
        wp_redirect(admin_url('admin.php?page=handl-utm-grabber-onboarding'));
        exit;
    }
}
add_action('admin_init', 'handl_utm_grabber_v3_redirect');